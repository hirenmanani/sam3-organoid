...
IOU_THRESHOLD = 0.4  # for deduplication

# Best prompts from our test
...
print(f"After shape filtering: {len(filtered)}")

# Deduplicate using IoU (remove overlapping detections from different prompts)
def compute_iou(m1, m2):
    inter = np.logical_and(m1, m2).sum()
    union = np.logical_or(m1, m2).sum()
    return inter / union if union > 0 else 0

# Sort by score descending, keep best non-overlapping
...
for candidate in filtered:
    ...
    for kept_item in kept:
        if compute_iou(m_c, kept_item[0]) > IOU_THRESHOLD:
            overlap = True
            break
    if not overlap:
...
ax.set_title(f"SAM 3 Final — {len(kept)} vesicles detected (multi-prompt + shape filtering)", fontsize=11)
plt.axis('off')
plt.tight_layout()
plt.savefig("output_final.png", dpi=200, bbox_inches='tight')
print("Saved to output_final.png")
plt.show()
